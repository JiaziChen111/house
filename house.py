...
rewritten according to PEP8 on 13 Jan. 2017
...
class state:
    ...
    def update(self, par, n=0):
        ...
        rs = par.ALP * (self.KS / self.L)**(par.ALP - 1) - par.DEL
        self.bq = self.BQ
        if self.transition:
            self.r[1:] *= (1 + par.SSr * (self.KD[1:] - self.KS[1:]) * 2 /
                           (self.KD[1:] + self.KS[1:]))
            self.q[:-1] *= (1 + par.SSq * (self.HD[1:] - self.HS[1:]) /
                            self.HS[1:])
            self.s *= (1 + par.SSs * self.RT / self.HS)
        else:
            self.r *= (1 + random() * par.SSr * (self.KD - self.KS) *
                       2 / (self.KD + self.KS))
            self.q *= (1 + random() * par.SSq * (self.HD - self.HS) / self.HS)
            self.s *= (1 + random() * par.SSs * self.RT / self.HS)
        self.KD = ((self.r + par.DEL) / par.ALP)**(1 / (par.ALP - 1)) * self.L
        self.w = (((self.r + par.DEL) / par.ALP)**(par.ALP / (par.ALP - 1)) *
                  (1 - par.ALP))
        ...
        if self.transition:
            ...
            if par.savgol_windows > 1:
                self.r[1:] = savgol_filter(r0[1:], par.savgol_windows,
                                           par.savgol_order)
                self.q = savgol_filter(q0, par.savgol_windows,
                                       par.savgol_order)
                self.s = savgol_filter(qr0, par.savgol_windows,
                                       par.savgol_order)
            ...
            title = ("TP at d%i" % (time.day) + "h%i" % (time.hour) +
                     " after %i loops" % (n))
            filename = title + '.png'
            ...
            ax5.plot((self.CO + self.s * self.CR) / sum(self.pop, axis=1), '.',
                     label='expenditure')
            ax6.plot((self.DI - self.CO) / self.DI, '.', label='saving rate')
            ax6.plot((self.DI - self.CO - self.s * self.CR) / self.DI, '.',
                     label='saving rate with rent')
    ...
    def converged(self):
        return (max(absolute(self.KD - self.KS)) / max(self.KD) < self.tol and
                max(absolute(self.HD - self.HS)) / max(self.HS) < self.tol and
                max(absolute(self.RT) / max(self.HS)) < self.tol)

    def prices(self, n=0, t=0):
        print("n=%2i" % (n), "t=%3i" % (t), "r=%2.2f%%" % (self.r[t] * 100),
              "Kd=%+3.2f%%," % ((self.KD[t] - self.KS[t]) * 2 /
                                (self.KD[t] + self.KS[t]) * 100),
              "q=%2.2f," % (self.q[t]),
              "HD=%+3.2f%%," % ((self.HD[t] - self.HS[t]) / self.HS[t] * 100),
              "s=%1.2f," % ((self.s[t])),
              "Rd=%+3.2f%%," % ((self.RT[t]) / self.HS[t] * 100),
              "bq=%2.2f%%," % (self.BQ[t] * 100),
              "b=%2.2f%%," % (self.b[t] * 100))
    ...
    def plot(self, par, t=0, yi=15, yt=70, ny=4):
        ...
        title = ('aN=%3i' % (aN) + ' hN=%2i' % (hN) + ' r=%2.2f%%'
                 % (self.r[t] * 100) + ' q=%2.2f' % (self.q[t]) +
                 ' s=%2.2f%%' % (self.s[t] * 100))
        if self.transition:
        ...
        gini_l = 1 - sum([(ly[i] + ly[i - 1]) * (lx[i] - lx[i - 1])
        ...
        gini_t = 1 - sum([(ly[i] + ly[i - 1]) * (lx[i] - lx[i - 1])
                          for i in range(1, len(lx))])
        ...
        gini_i = 1 - sum([(ly[i] + ly[i - 1]) * (lx[i] - lx[i - 1])
                          for i in range(1, len(lx))])
...
class cohort:
    ...
    def __init__(self, par, ss=0, sy=0):
        ...
        # v[y,h,j,i] is the value of y-yrs-old agent with asset i and prod.
        # j, house h
        ...
        for each age, distribution over all productivities and assets
        add up to 1 """
    ...
    def optimalpolicy(self, par, ps):
        ...
        yN, hN, zN, aN = par.yN, par.hN, par.zN, par.aN
        hh, aa = par.hh, par.aa
        ps = concatenate((repeat(ps[:, 0][:, newaxis],
                                 max(yN - ps.shape[1], 0), axis=1), ps),
                         axis=1)
        r, w, q, s, b, bq, THE, Tr, Tw, Th = ps
        sp = self.sp
        sy = self.sy

        """ev[y,nh,j,ni] is the expected value when next period
        asset ni and house hi"""
        ...
        """ inline functions: utility and income adjustment
        by trading house """
        """!!!!!!!!!!!!!!!!!!  from here  !!!!!!!!!!!!!!!!!!!"""
        def util(c, r, h):
            return (((maximum(c, 1e-15)**(1 - par.PSI) *
                      maximum(h + r, 1e-15)**par.PSI)**(1 - par.SIG)) /
                    (1 - par.SIG))
        """!!!!!!!!!!!!!!!!!!  to here  !!!!!!!!!!!!!!!!!!!!!"""
        def hinc(h, nh, q):
            return (h - nh) * q - par.TC * h * q * (h != nh)
        """ y = -1 : just before the agent dies """
        for h in range(hN):
            for z in range(zN):
                B = (aa * (1 + (1 - Tr[-1] * (aa > 0)) * r[-1]) +
                     hinc(hh[h], hh[0], q[-1]) - hh[h] * q[-1] * Th[-1] +
                     w[-1] * par.ef[-1, z] * (1 - THE[-1] - Tw[-1]) +
                     b[-1] + bq[-1])
                ...
                """!!!!!!!!!!!!!!!!!!  to here  !!!!!!!!!!!!!!!!!!!!!"""
                inc[-1, h, z] = (- s[-1] * minimum(rtt[-1, h, z], 0) -
                                 Th[-1] * q[-1] * hh[h] + b[-1] + bq[-1] +
                                 aa * (1 - Tr[-1] * (aa > 0)) * r[-1])
        ...
        for y in range(-2, sy - yN - 1, -1):
            for h in range(hN):
                for z in range(zN):
                    vt = zeros((hN, aN, aN))
                    for nh in range(hN):
                        p = (aa * (1 + (1 - Tr[y] * (aa > 0)) * r[y]) +
                             b[y] * (y >= -par.R) + w[y] * par.ef[y, z] *
                             (1 - THE[y] - Tw[y]) + bq[y] +
                        ...
                        rt = (B - c) / s[y]
                        vt[nh] = (util(c, rt, hh[h]) +
                                  par.BET * sp[y + 1] * ev[y + 1, nh, z])
                        cv = ((c <= 0.0) | (rt + hh[h] <= 0.0) |
                              (aa +
                               minimum(par.ltv * hh[nh] * q[y + 1],
                                       par.dti * par.pi[z].dot(ei[y + 1, nh]) /
                                       (1 + r[y + 1])) < 0.0))
                        vt[nh][cv] = -float("inf")
                        """!!!!!!!!!!!!!!!!!!  to here  !!!!!!!!!!!!!!!!!!!"""
                    for a in range(aN):
                        ...
                        self.h[y, h, z, a], self.a[y, h, z, a] = \
                            unravel_index(vt[:, a, :].argmax(),
                                          vt[:, a, :].shape)
                        self.v[y, h, z, a] = vt[self.h[y, h, z, a], a,
                                                self.a[y, h, z, a]]
                        B = (aa[a] * (1 + (1 - Tr[y] * (aa[a] > 0)) * r[y]) +
                             b[y] * (y >= -par.R) + bq[y] +
                        ...
                        inc[y, h, z, a] = \
                            (- s[y] * minimum(rtt[y, h, z, a], 0) +
                             b[y] * (y >= -par.R) + bq[y] +
                             aa[a] * (1 - Tr[y] * (aa[a] > 0)) * r[y] +
                             w[y] * par.ef[y, z] * (1 - THE[y] - Tw[y]) -
                             Th[y] * q[y] * hh[h])
                        ei[y, h, z, a] = \
                            (inc[y, h, z, a] +
                             par.pi[z].dot(ei[y + 1, self.h[y, h, z, a], :,
                                              self.a[y, h, z, a]]) /
                             (1 + r[y + 1]))
        ...
        for y in range(sy + 1, yN):
            for h in range(hN):
                for z in range(zN):
                    for a in range(aN):
                        mu[y, self.h[y - 1, h, z, a], :,
                           self.a[y - 1, h, z, a]] += (mu[y - 1, h, z, a] *
                                                       par.pi[z])
...
def fss(par, k, c, N=300):
    """Find Old and New Steady States with population growth rates
    ng and ng1"""
...
# separate the procedure of finding optimal policy of each cohort
...
def sub1(t, vmu, vc, vrt, vin, ps, c0, c1, par):
    ...
    if sy > 0:
        c.mu_sy = c0.vmu.reshape(yN, hN * zN * aN)[sy]
    if t < T - 1:
...
def tran(par, kt, c0, c1, N=5):
    ...
    for n in range(N):
        ...
        for t in range(T):
            p = Process(target=sub1, args=(t, VM[t], VC[t], VRT[t], VIN[t],
                                           kt.ps, c0, c1, par))
            p.start()
            jobs.append(p)
            # if t % 40 == 0:
...
if __name__ == '__main__':
    par = params(T=200, aN=60, aL=-10, aH=50, hL=0.0, hH=1.5, hs=0.2, hN=5,
                 zN=18, ALP=0.32, PSI=0.4, DEL=0.08, BET=0.998, SIG=2.0,
                 Tr=0.10, Tw=0.15, Th=0.01, THE=0.1, ZET=0.15, TC=0.02,
                 dti=7 / 10, ltv=5 / 10, spm=0.1,
                 savgol_windows=51, savgol_order=1,
                 SSr=0.12, SSq=0.12, SSs=0.09, gs=2.0, tol=0.001)
    par.values()

    # k0 = state(par, ss=0, r_0=0.06481, q_0=13.942, s_0=0.9635, bq_0=0.0554)
    k0 = state(par, ss=0, r_0=0.06481, q_0=13.942, s_0=0.9635, bq_0=0.0554)
    c0 = cohort(par, ss=0)
