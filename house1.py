...
class state:
    ...
    def __init__(self, params, r_init=0.05, r_term=0.05, Bq_init=0, Bq_term=0,
        ...
        self.r[0:T-2*yN] = linspace(r_init,r_term,T-2*yN)
        self.q[0:T-2*yN] = linspace(q_init,q_term,T-2*yN)
        self.Bq[0:T-2*yN] = linspace(Bq_init,Bq_term,T-2*yN)
    ...
    def update_prices(self, n=0):
        ...
        K0 = self.K   # capital demand that is consistent with the interest rate
        ...
        if self.T > 1:
            self.q[:-1] = self.q[:-1]*(1+self.eps*(self.Hd[1:]-self.Hs[1:]))
        else:
            self.q = self.q*(1+self.eps*(self.Hd-self.Hs))
        rmin = min(self.r_init,self.r_term) - 0.005
        rmax = max(self.r_init,self.r_term) + 0.005
        qmin = min(self.q_init,self.q_term) - 1.0
        qmax = max(self.q_init,self.q_term) + 1.0
        if self.T > 1:
            r0 = self.r
            q0 = self.q
            if self.filter_on == 1:
                ...
                self.r[1:] = savgol_filter(r0[1:], self.savgol_windows, self.savgol_order)
            ...
            ax.tick_params(labelcolor='w', top='off', bottom='off', left='off',
                            right='off')
            ax1.plot(self.K1[1:]/sum(self.pop[1:],axis=1),label='K1')
            ax1.plot(self.K[1:]/sum(self.pop[1:],axis=1),label='Updated')
            ax1.plot(K0[1:]/sum(self.pop[1:],axis=1),label='K0')
            ax2.plot(self.Hd[1:]/sum(self.pop[1:],axis=1),label='Demand')
            ax2.plot(self.Hs[1:]/sum(self.pop[1:],axis=1),label='Supply')
            ax3.plot(self.r[1:],label='smoothed')
            ax3.plot(r0[1:],label='updated')
            ax3.plot(self.r1[1:],label='r1')
            ...
            ax5.plot(self.Prod[1:]/sum(self.pop[1:],axis=1),label='per capita Production')
            ax5.plot(self.Cons[1:]/sum(self.pop[1:],axis=1),label='per capita Consumption')
            ax6.plot((self.DI[1:]-self.Cons[1:])/self.DI[1:])
    ...
    def print_prices(self, n=0, t=0):
        print "n=%2i"%(n)," t=%3i"%(t),"r=%2.2f%%"%(self.r[t]*100),\
              ...
              "Ks=%3.1f%%,"%((self.K1[t]-self.K[t])/self.K[t]*100),"q=%2.2f,"%(self.q[t]),\
    ...
    def plot(self, t=0, yi=0, yt=78, ny=7):
        ...
        title = 'psi=%2.2f'%(self.psi) + ' aN=%3i'%(self.aN) \
                + ' hN=%2i'%(self.hN) + ' r=%2.2f%%'%(self.r[t]*100) \
...
class cohort:
    """ This class is just a "struct" to hold the collection of primitives defining
    a generation """
    def __init__(self, params, sy=0, a0=0):
        self.beta, self.sigma, self.psi = params.beta, params.sigma, params.psi
        self.R, self.W, self.sy = params.R, params.W, sy
        self.yN = yN = params.yN
        self.syr = sy - yN
...
def sub1(t,vmu,vc,prices,c0,c1,params):
    sy = max(params.yN-t-1,0)
    c = cohort(params, sy=sy)
    if sy > 0:
        c.mu_sy = c0.vmu.reshape(c.yN,c.hN*c.zN*c.aN)[sy]
    if t < params.T-1:
...
def tran(params, kt, c0, c1, N=5):
    ...
    for n in range(N):
        ...
        for t in range(T):
            p = Process(target=sub1, args=(t,VM[t],VC[t],kt.prices,c0,c1,params))
...
def find_2ss(pg0=1.012, pg1=1.000):
    start_time = datetime.now()
    par = params(T=1, psi=0.5, delta=0.06, aN=60, aL=-10, aH=60,
            Hs=0.3, hN=7, tol=0.001, phi=0.75, eps=0.075, tcost=0.02, gs=2.0,
            alpha=0.36, tau=0.2378, theta=0.1, zeta=0.3, sp_multiplier=0.1,
            savgol_windows=51, savgol_order=1, filter_on=1,
...
def find_trans(phi=0.75, eps=0.02, T=240, windows=7, N=5):
    start_time = datetime.now()
    ...
    par1.phi = phi
    par1.eps = eps
    par1.savgol_windows = windows
    par1.pg, par1.pg_change = par0.pg, par1.pg-par0.pg
    par1.sp0 = par0.sp0
    # par1.sp1 = par1.sp1
    par1.T = T
    kt = state(par1, r_init=k0.r, r_term=k1.r, q_init=k0.q, q_term=k1.q,
                                                Bq_init=k0.Bq, Bq_term=k1.Bq)
    kt, mu, vc = tran(par1, kt, c0, c1, N=N)

    for t in linspace(0,par1.T-1,10).astype(int):
        kt.plot(t=t,yi=10,ny=5)
    with open('transition.pickle','wb') as f:
        pickle.dump([kt, par1], f)
    end_time = datetime.now()
    print 'Total Time: {}'.format(end_time - start_time)


def find_trans_cont(phi=0.80, eps=0.02, windows=3, N=5):
    start_time = datetime.now()
    ...
    kt, mu, vc = tran(par, kt, c1, N=N)

    for t in linspace(0,par.T-1,10).astype(int):
        kt.plot(t=t,yi=10,ny=5)
    with open('transition1.pickle','wb') as f:
...
if __name__ == '__main__':
    # k0, c0, k1, c1 = find_2ss(pg0=1.012, pg1=1.000)
    find_trans(phi=0.75, eps=0.02, T=240, windows=7, N=15)
    # find_trans_cont(phi=0.80, eps=0.02, windows=3, N=5)
